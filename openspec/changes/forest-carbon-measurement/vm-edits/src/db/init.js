...
function getDb() {
  if (!db) {
    ...
    // §38 重測沙箱：對既有 tmp_frames 跑新 pipeline，結果獨立於 trees 表
    db.exec(`
      CREATE TABLE IF NOT EXISTS dryrun_runs (
        ...
        tmp_frames_dir TEXT,
        all_card_frames_json TEXT,
        selected_frame_idxs_json TEXT,
        gemini_frames_json TEXT,
        median_json TEXT,
        dbh_cm REAL,
        winner_path TEXT,
        baseline_manual_tape_cm REAL,
        baseline_path0_cm REAL,
        baseline_pathA_cm REAL,
        error_vs_baseline_pct REAL,
        error_vs_old_pathA_pct REAL,
        notes TEXT
      );
      CREATE INDEX IF NOT EXISTS idx_dryrun_runs_tree ON dryrun_runs(tree_id);
      CREATE INDEX IF NOT EXISTS idx_dryrun_runs_run_at ON dryrun_runs(run_at);
    `)

    // §38.2 dryrun 雙路徑（legacy §32 + opencv §35.7 並跑，挑誤差較低當 pathA winner）
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN path0_dbh_cm REAL;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_legacy_dbh_cm REAL;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_legacy_ratio REAL;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_legacy_confidence REAL;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_legacy_frame_idxs_json TEXT;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_legacy_raw_json TEXT;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_opencv_dbh_cm REAL;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_opencv_frame_idxs_json TEXT;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN error_pathA_legacy_pct REAL;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN error_pathA_opencv_pct REAL;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_winner_strategy TEXT;`) } catch (_) {}
    try { db.exec(`ALTER TABLE dryrun_runs ADD COLUMN pathA_winner_dbh_cm REAL;`) } catch (_) {}
  }
  return db
}
