# §27.9 retry: 9 missing videos after deploy.sh fix
...
$LOG = 'C:\Users\yuchi\openspec\changes\forest-carbon-measurement\batch-upload-missing9.log'

$missing = @('IMG_5789.mov','IMG_5790.mov','IMG_5794.mov','IMG_5795.mov','IMG_5803.mov','IMG_5805.mov','IMG_5811.mov','IMG_5814.mov','IMG_5815.mov')
...
foreach ($name in $missing) {
    ...
    if (Test-Path -LiteralPath $p) {
        ...
        Write-Host ('MISSING ON DISK: ' + $name) -ForegroundColor Yellow
    }
}
$total = $files.Count
Write-Host ('Found ' + $total + ' of 9 missing videos. SRC=' + $SRC)
('=== batch-upload-missing9 start ' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + ' ===') | Out-File $LOG -Append -Encoding utf8
