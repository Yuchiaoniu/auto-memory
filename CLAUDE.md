...
使用者在討論想法、探索問題、評估方向、還沒決定要做什麼時，
主動呼叫 Skill(openspec-explore) 進入探索模式。

使用者說「就這樣做」「開始實作」「來做吧」「決定了」等確認實作的話，
呼叫 Skill(openspec-propose) 產生提案；提案確認後呼叫 Skill(opsx:apply) 開始實作。
